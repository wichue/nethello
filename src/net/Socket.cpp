    ...
    while (_enable_recv) {
        ...
        if (nread == 0) {
            if (sock->type() == SockNum::Sock_TCP) {
                ...
                emitErr(SockException(Err_eof, "end of file"));//chw
            }
            return ret;
        }
...
void Socket::SetSndType(SEND_TYPE type)
{
    ...
    case SEND_BUFF:
...

        {
            // 定时检测_SndBuffer
            weak_ptr<Socket> weak_self = shared_from_this();
            _snd_timer_b = std::make_shared<Timer>(_snd_timeout_b,[weak_self]() {
                auto strong_self = weak_self.lock();
                if (!strong_self) {
                    return false;
                }
...
uint32_t Socket::timer_b()
{
    LOCK_GUARD(_mtx_sock_fd);
    if (_snd_ticker_b.elapsedTime() > _flush_b_times)
    {
        flush_b();
    }
