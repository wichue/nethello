...
LogContext::LogContext(bool bCR, LogLevel level, const char *file, const char *function, int line, const char *module_name, const char *flag)
        : _bCR(bCR), _level(level), _line(line), _file(getFileName(file)), _function(getFunctionName(function)),
          _module_name(module_name), _flag(flag) {
    gettimeofday(&_tv, nullptr);
    _thread_name = getThreadName();
}
...
LogContextCapture::LogContextCapture(bool bCR, Logger &logger, LogLevel level, const char *file, const char *function, int line, const char *flag) :
        _ctx(new LogContext(bCR,level, file, function, line, s_module_name.c_str() ? s_module_name.c_str() : "", flag)), _logger(logger) {
}
