...
int EventLoop::addEvent(int fd, int event, PollEventCB cb) {
    // TimeTicker();
    if (!cb) {
...
int EventLoop::delEvent(int fd, PollCompleteCB cb) {
    // TimeTicker();
    if (!cb) {
        cb = [](bool success) {};
    }
...
int EventLoop::modifyEvent(int fd, int event, PollCompleteCB cb) {
    // TimeTicker();
    if (!cb) {
        cb = [](bool success) {};
    }
...
Task::Ptr EventLoop::async_first(TaskIn task, bool may_sync) {
...
inline void EventLoop::onPipeEvent(bool flush) {
    char buf[1024];
    int err = 0;
    if (!flush) {
       for (;;) {
...
// SocketRecvBuffer::Ptr EventLoop::getSharedBuffer(bool is_udp) {
...
thread::id EventLoop::getThreadId() const {
...
void EventLoop::runLoop(bool blocked, bool ref_self) {
    if (blocked) {
        ...
        _loop_thread = new thread(&EventLoop::runLoop, this, true, ref_self);
...
EventLoop::DelayTask::Ptr EventLoop::doDelayTask(uint64_t delay_ms, function<uint64_t()> task) {
